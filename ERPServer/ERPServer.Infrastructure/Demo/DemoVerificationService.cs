    ...
    ) : IDemoVerificationService
{
        ...
        string email, string? ipAddress, string? userAgent, CancellationToken cancellationToken = default)
    {
        ...
        // CodeResendSeconds dolmadan isteyemiyor.
    ...
    private Task SendMailAsync(string email, string code, CancellationToken cancellationToken)
    {
        string body = MailTemplate.Wrap(
            ...
            MailTemplate.Paragraph(
                "Tezgah demosunu başlatmak için aşağıdaki kodu uygulamaya girin.")
            ...
            string.IsNullOrWhiteSpace(_mail.ClientBaseUrl) ? DemoOptions.DefaultContactUrl : _mail.ClientBaseUrl);

        return fluentEmail
            .To(email)
            .Subject($"Tezgah demo doğrulama kodunuz: {code}")
    ...
    private static bool TryNormalize(string? email, out string normalized, out string display)
    {
        ...
        // Nokta içermeyen bir alan adı (ali@localhost gibi) geçerli sayılır ama
        // posta kutusu değildir.
